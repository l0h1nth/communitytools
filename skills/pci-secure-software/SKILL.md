...
1. Ask Codex to use `$pci-secure-software` with `scope_file=projects/compliance/pci-sss-scope.md` and `mode=intake`; record the returned `engagement_dir` and applicable objectives.
2. For each applicable objective `O`, use the skill with `engagement_dir`, `mode=assess`, and `objective=O`.
3. Use the skill with `engagement_dir` and `mode=report` to verify and report.
...
The deterministic engine lives in `tools/pci-sss/` (`applicability.py`, `citation_verify.py`, `coverage_gate.py`, `aggregate.py`, `validate_catalog.py`, `build_catalog.py`); the orchestration lives in `workflows/pci-compliance.js`.
